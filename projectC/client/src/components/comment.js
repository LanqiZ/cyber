...
const API_URL = 'https://localhost:3001'; // Define the API URL
...
const [editCommentId, setEditCommentId] = useState(null);
  ...
  const handleCommentSubmit = async (postId, commentContent, commentAuthor) => {// Function to handle comment submission
      ...
      const response = await fetch(`https://localhost:3001/api/posts/${postId}/comments`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
    ...
    //       };
      const handlePostEditSubmit = (e) => {// Function to handle post edit submission
        ...
        if (post.title !== "" && post.author !== "" && post.content !== "") {
          fetch(`https://localhost:3001/api/posts/${editPostId}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
...
fetch(`https://localhost:3001/api/posts/${post._id}/upvote`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
>
  ...
  variant="contained"
    ...
    const newVotes = comment.vote + 1;
      ...
      var tokenuser = localStorage.getItem('token')
      console.log(tokenuser)
fetch(`https://localhost:3001/api/posts/${comment._id}/upvote/comment`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
